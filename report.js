...
if (!payloadText) {
  ...
  try {
    ...
    const claims = trips.reduce((sum, trip) => sum + claimAmount(trip), 0);
    ...
    $("#report-claim").textContent = formatMoney(claims);
    ...
    $("#report-rows").innerHTML = trips.map((trip) => {
      ...
      return `<tr><td>${escapeHtml(formatDate(trip.date))}</td><td>${escapeHtml(work)}</td><td>${escapeHtml(route)}</td><td>${escapeHtml(trip.vehicle || "—")}</td><td>${escapeHtml(formatKm(totalDistance(trip)))}</td><td>${trip.rateCents ? `${escapeHtml(trip.rateCents)}¢/km` : "—"}</td><td>${trip.rateCents ? escapeHtml(formatMoney(claimAmount(trip))) : "—"}</td></tr>`;
    }).join("");
  } catch {
    $("#report-content").hidden = true;
    $("#report-message").hidden = false;
