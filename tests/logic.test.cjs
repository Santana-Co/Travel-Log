...
test("uses local calendar boundaries across Australian time zones", () => {
  ...
  const cases = [
    ...
    ["Australia/Sydney", "2026-10-03T13:30:00.000Z", "2026-10-03"],
...
test("defaults new and duplicated trips to the local day while preserving edits", () => {
...
test("validates filter ranges and trip bounds", () => {
  ...
  assert.match(validateTrip({ ...validTrip, endDate: "2026-02-30" }), /Journey end date/i);
  assert.equal(validateTrip({ ...validTrip, date: "2024-02-29", endDate: "2024-02-29" }), "");
